...
│   ├── run-tests.sh                 # Test runner (123 unit/integration files); exit 2 = nothing checked
...
**Observe the dispatch class at Stop; never predict it at dispatch time.** FEAT-033/ADR-027 (#218) closes the gap the paragraph above named and deferred: the in-flight hold used to classify a dispatch from `tool_input.run_in_background` captured at `PreToolUse` — a field Claude Code omits from the exposed Agent schema in fork mode, so on that host class every marker recorded `background: "missing"` and the hold never engaged. The answer exists one event later, in the Stop payload's `background_tasks[]`, read once via `scripts/lib/hook-stdin.sh`'s bounded, EOF-independent `read -r -d '' -t` — a bash builtin, so no `timeout(1)` is needed, closing the #155-class never-EOF-stdin deadlock the ten existing `cat`-behind-`[ -t 0 ]` readers still carry live rather than reopening it. Liveness is two independent counts, never one filter (ruling Q2): `LIVE` (`type=="subagent"` AND an allowlisted `running`/`pending` status) gates the HOLD; `SUBAGENT_PRESENT` (type only, status-blind) gates a CANDIDATE. A status neither count recognises still increments the status-blind present count while leaving the live one at zero, so the tick reads `SUBAGENT_PRESENT > 0` with `LIVE == 0`: what is absent is a DISPOSITION, not a count, and that third state gets its own record instead of silence (`stop_gate reason: "in_flight_present_not_live"`, TASK-013), so an unrecognised future status can never be misread as either "live" or "none" — §15/ADR-009's looked-vs-never-looked doctrine applied to the filter itself. An empty registry (`SUBAGENT_PRESENT == 0`) is DETECT-ONLY (ruling Q3), and only for the markers that branch can reach — unnamed (`named != "true"`) and not proven foreground (`background != "false"`), because those two classes are disposed of by their own recorded field rather than by the payload: `stop_gate reason: "in_flight_orphan_candidate"` records the observation and the marker is LEFT IN PLACE, not quarantined, until ADR-027's numeric bar (≥20 such events across ≥2 objectives with zero contradicting evidence) is met, so the arm ships a measurement instrument before it ships an action. The #245 ruling splits that PROVEN class by strength of proof: `background: "false"` is a mechanical impossibility — a synchronous dispatch cannot span a Stop — so it is quarantined on a live tick exactly as on any other and the hold's `units` never names one, while `named: "true"` proof is merely contractual (a named dispatch can be genuinely background AND live), so on a `LIVE > 0` tick it is held and named rather than quarantined. Because a hold's `exit 0` IS process exit in the canonical unattended shape (`claude -p` has no idle state to resume into), the hold that now engages for the first time is bounded by a Q1 valve — `_IN_FLIGHT_HOLD_CAP=1`, a script constant, ledgered by marker-set fingerprint under `nazgul/logs/.in-flight-holds/` — that refuses a second hold on an unchanged marker set (`stop_gate reason: "in_flight_hold_budget_exhausted"`). The honest boundary ships with it: the harness's task-notification wake this hold now depends on remains an OWED, UNOBSERVED probe (R2) on this host class — exactly why the valve exists instead of an unbounded hold — and the release carrying it requires one supervised objective before any AFK or overnight run.
...
**A green test is evidence only after it proved it can turn red.** FEAT-028/ADR-019 makes that thesis mechanical: `scripts/red-run.sh` runs a task's changed tests against its pre-change Base SHA, and the same shared transition library that verifies commit evidence blocks IMPLEMENTED when required red-run evidence is missing or corrupt. Fixtures at load-bearing seams are captured from real producers, carry machine-checked provenance and mutation pins, guards are dogfooded on this shell-heavy repository's own language and real hook envelopes, and every checking entry point reports `scanned / skipped / checked / findings` so “looked and found none” cannot collapse into “never looked.” The registry of bound entry points lives in `RULES.md` §15 (sixteen entry points are bound: `scripts/self-audit.sh` enrolled by FEAT-029/TASK-012, `scripts/audit-agent-state-paths.sh` and `tests/test-dispatch-brief-contract.sh` by FEAT-030, `scripts/close-objective.sh` by FEAT-031/TASK-011, `tests/test-messaging-posture.sh` by FEAT-032/TASK-012, `tests/test-doc-contract-fields.sh` by FEAT-031/TASK-016, `scripts/lib/task-transition-guard.sh`'s red-run scans by FEAT-031/TASK-019, the shared review-unit classifier `scripts/lib/review-file-class.sh` by FEAT-031/TASK-034, relocated there from `review-evidence.sh` by TASK-035 when the same rule and the same printf came to answer for BOTH of the DONE gate's passes, `tests/test-shared-ignore-coverage.sh` by FEAT-034, and `tests/test-manifest-write-integrity.sh` — one walk of `scripts/**` printing under two tokens, `manifest-writers` and `status-readers`, covered only when BOTH conform — by FEAT-036/TASK-015 — this sentence stood at `eleven` for two enrolments, so the number is now DERIVED from §15's own bullet by `tests/test-doc-contract-fields.sh` and a sixteenth enrolment turns this file red rather than leaving it quietly stale; that is what FEAT-036 was, and the derivation did turn it red) and `tests/test-coverage-honesty.sh` fails if any enumerated one is never driven — it previously cited a per-objective TRD section that was archived out from under the citation. `guards.red_run_evidence` (schema v36, default `true`) suppresses the block only; the diagnostic and `red_run_missing` event remain.
...
```bash
tests/run-tests.sh                    # Run all unit/integration tests (123 files)
