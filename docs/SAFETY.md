...
- **In-flight dispatch hold**: while a dispatched BACKGROUND `Agent` is still running, `scripts/stop-hook.sh` takes an ALLOWED, uncounted stop instead of burning an iteration — a marker written on dispatch (`scripts/in-flight-marker.sh`, PreToolUse/Agent) and cleared on completion (`scripts/subagent-stop.sh`) is the sole trigger, so the hold never depends on orchestrator memory. `current_iteration` and `safety.consecutive_failures` stay untouched while the hold is taken; a marker whose recorded dispatch class is not provably background is not provably awaited work and is quarantined by cost-weighing under `nazgul/in-flight/quarantine/` (`stop_gate` `reason: "in_flight_orphan"`) while the loop continues normally. Where the class was not observable at all, this is recorded as `in_flight_unverifiable` and the marker is deliberately NOT quarantined — it is left in `nazgul/in-flight/` because it is usually a healthy background dispatch rather than a leak, the move is irreversible, and destroying it would foreclose #218's fix (#218); a stale marker (older than `guards.in_flight_stale_minutes`, default 30) is surfaced loudly on stderr and in telemetry rather than held on or silently dropped. Kill-switched by `guards.in_flight_hold` (default `true`) — see `docs/CONFIGURATION.md`
