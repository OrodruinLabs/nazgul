...
- **Gate-triggered stops** — `stop_gate` (eleven stop_gate reasons, a closed set: `afk_timeout`, `comment_verifier_marker_unverified`, `in_flight_hold`, `in_flight_orphan`, `in_flight_stale`, `in_flight_unverifiable`, `merge_evidence_undecided`, `review_validator_defect`, `stack_register_after_pr_failed`, `stack_submit_no_branch`, `stacking_unavailable`) fires whenever a gate ends or holds a turn in `scripts/stop-hook.sh`, `scripts/worktree-utils.sh` or `scripts/lib/stack-utils.sh` (three of the eleven are worktree and stack emissions, not stop-hook ones), so the telemetry always shows a mechanism acted rather than a bare `exit 0` — see `docs/CONFIGURATION.md` → Telemetry Bus → Event Types for what each means
