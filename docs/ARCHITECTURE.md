...
- **Gate-triggered stops** — `stop_gate` (nineteen stop_gate reasons, a closed set: `afk_timeout`, `comment_verifier_marker_unverified`, `in_flight_hold`, `in_flight_hold_budget_exhausted`, `in_flight_hold_unbudgetable`, `in_flight_orphan`, `in_flight_orphan_candidate`, `in_flight_orphan_unattributable`, `in_flight_present_not_live`, `in_flight_stale`, `in_flight_unverifiable`, `manifest_write_refused`, `merge_evidence_undecided`, `reconciliation_annotation_incomplete`, `reconciliation_quarantine_refused`, `review_validator_defect`, `stack_register_after_pr_failed`, `stack_submit_no_branch`, `stacking_unavailable`) fires whenever a gate ends or holds a turn in `scripts/stop-hook.sh`, `scripts/worktree-utils.sh` or `scripts/lib/stack-utils.sh` (three of the nineteen are worktree and stack emissions, not stop-hook ones), so the telemetry always shows a mechanism acted rather than a bare `exit 0` — see `docs/CONFIGURATION.md` → Telemetry Bus → Event Types for what each means
