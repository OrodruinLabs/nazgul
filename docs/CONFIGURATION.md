...
- **aggregate_unit_blocked_hold** — the sibling record to `aggregate_board_cancelled_carveout`, bound to the SCAN rather than a dispatch: a `group`/`feature` review unit's scan found every remaining task `BLOCKED`, so nothing can be implemented to release it, and a held unit dispatches no board by construction — gating this on a dispatch would leave the deadlock permanently unrecorded; fields `unit`, `blocked_tasks`, `blocked`, `implemented`, `total`
