...
| `project.test_filter_template` | absent (never written by migration — see below) | A literal scoping-flag template with one `{filter}` placeholder (`"--filter {filter}"`, `"-run {filter}"`, `"-k {filter}"`), substituted verbatim, never `eval`ed. Absent is NOT one fixed default: `scripts/red-run.sh` re-derives it at run time from whatever `project.test_command` currently says — `--filter={filter}` when the command is empty or matches the `tests/run-tests.sh` convention, and a hard REFUSAL for any other runner, so a stale flag can never survive a runner switch a written value would have missed. |
...
- **aggregate_unit_blocked_hold** — the sibling record to `aggregate_board_cancelled_carveout`, bound to the SCAN rather than a dispatch: a `group`/`feature` review unit's scan found **one or more** of its remaining tasks `BLOCKED`. That alone is sufficient to make readiness unsatisfiable — `UNIT_TOTAL` counts every non-`CANCELLED` member and readiness requires `UNIT_IMPLEMENTED` to equal it, so a single `BLOCKED` task already means implementing the rest will not release the board. A held unit dispatches no board by construction — gating this on a dispatch would leave the deadlock permanently unrecorded, so the scan emits it directly. Because it is bound to the scan rather than a one-time transition, this event re-emits on every iteration for as long as the hold persists, intentionally — a consumer wanting the number of distinct holds should key on `blocked_tasks` changing between emissions, not on the occurrence count; fields `unit`, `blocked_tasks`, `blocked`, `implemented`, `total`
- **close_objective_refused** — `/nazgul:complete` refused to close a task outright, before any per-task work began; fields `task_id`, `reason`, `detail`, `pr`
- **close_objective_roster_unreadable** — a task's objective roster could not be read, so closure was refused for it rather than assumed; fields `feat_id`, `reason`, `pr`
- **close_objective_blocked** — the whole closer run halted before touching any manifest because the PR's own merge state was globally unusable; fields `pr`, `reason`, `result`, `detail`
- **close_objective_closed** — one task's manifest reached `DONE` through `/nazgul:complete`'s merge-evidence route; fields `task_id`, `from` (the pre-closure status), `pr`, `host`, `merge_commit`
- **close_objective_rollback_failed** — a task manifest was left carrying `## Merge Evidence` that this run's own later failure means no closure now stands behind — the single most dangerous residue this objective can produce (RULES.md §2); fields `count`, `pr`
- **close_objective_summary** — the terminal record of one `/nazgul:complete` run, printed beside its own §15 coverage line; fields `pr`, `result`, `feat_id`, `head_ref`, `scanned`, `skipped`, `closed`, `refused`
- **coverage_vacuous** — a checking entry point had candidates but checked none of them (RULES.md §15); shared verbatim by `scripts/lean-comments-guard.sh` and `scripts/close-objective.sh`'s own coverage line; fields `entry_point`, `scanned`, `skipped`, `refused`, plus the emitting entry point's own identifiers (e.g. `pr` for the closer)
- **review_gate_enforcement_skipped** — reactive per-task review enforcement did not run for an iteration because YOLO + `task_pr` mode reaches `DONE` by PR merge, not a local board; fields `reason` (`yolo_task_pr_mode`), `unchecked` (how many `DONE` tasks were left unchecked this iteration)
- **board_status_option_migration** — `scripts/board-sync-github.sh` added, read back, or failed to reconcile one or more "Nazgul Status" single-select options against GitHub Projects; fields `result` (a migration outcome, or `readback_failed` / `map_incomplete`), `detail`, `field_id`, `project`
- **plan_objective_stamped** — `scripts/stamp-plan-objective.sh` stamped, found already-bound, or refused to stamp `nazgul/plan.md`'s frontmatter `feat_id` — the corroboration RULES.md §2's merge-evidence gate reads; fields `action` (`stamped` / `already-bound` / `refused`), `feat_id`, `plan`, and either `roster_tasks` (stamped/already-bound) or `reason` (refused)
- **reconciliation_quarantine** / **gate_attribution** / **task_transition** — pre-date this objective and are documented in full at RULES.md §2 ("A quarantine is a typed integrity annotation") and §1 rule 8 respectively; listed here because they are emitted by `scripts/stop-hook.sh` / `scripts/lib/task-transition-guard.sh` and this list is now the closed, derived population of what those producers emit. `reconciliation_quarantine` fields: `task_id`, `kind`, `checkpoint_status`, `observed_status`, `since`, `action`. `gate_attribution` fields: `gate`, `writer`, `objective`, `marker`, `attempts`. `task_transition` fields: `task_id`, `from`, `to`
...
- **stack_halt_write_failed** — `_su_halt_stacking` could not persist `execution.stacking.halted`, or wrote it and failed to read it back as `true`. The loud form of the failure that matters most here: stacking is NOT halted despite the warning that says it should be, so a human must set the flag by hand before the next tick; fields `reason`, `config`, and `verify` (the value re-read, when the write itself succeeded)
- **stack_registry_duplicate_feat_id** — two or more `stack.layers[]` entries share one `feat_id`, so marking a layer merged marked only the FIRST and left the rest open for a human; fields `feat_id`, `count`
- **stack_rework_file_failed** — a `CHANGES_REQUESTED` review was detected but its p1 `stack-rework` inbox item could not be written, so the rework the heartbeat would have picked first was DROPPED; the counterpart to `stack_rework_filed`, and named separately because a filing that failed must never read as one that succeeded; fields `pr`, `review_id`, `feat_id`, `file`
