...
- [ ] **Step 1: Write failing tests**: (a) enabled + fake collector → events since cursor POSTed once with `batch_start_offset`, cursor advances only on 2xx; (b) collector down → exit 0, cursor unmoved; (c) `collector.enabled: false` → no request attempted (fixture asserts zero hits); (d) malformed cursor file → resend from start, warning on stderr; (e) snapshot payload contains no `prompt_head` values; (f) `http://` non-loopback URL → refused, zero requests, one stderr warning.
...
**Interfaces:**
- Produces HTTP API: `POST /ingest/events` (JSONL body → `events` table; idempotent via unique `(registry_key, offset)` + `INSERT OR IGNORE`, so redelivered batches are harmless), `POST /ingest/snapshot` (→ `snapshots` upsert-latest + history), `GET /fleet` (one row per registry_key: latest snapshot summary + last event + in-flight agents), `GET /loops/:key/events?since=<offset>` (same monotonic offset as ingest keys). Auth: `Authorization: Bearer <pre-shared key>` when `COLLECTOR_TOKEN` env set; without a token the listener binds LOOPBACK ONLY and refuses to start on a non-loopback interface (PR #80 review).
...
- [ ] **Step 1: Write failing ingest tests** (node:test + supertest-style fetch against an ephemeral server + `:memory:` DB): events batch lands with correct rows; SAME batch redelivered → no duplicate rows (idempotency); snapshot upserts latest; bad JSON → 400 without crash and without partial batch insert (atomic); token required when env set; no token + non-loopback bind requested → server refuses to start.
