...
**Interfaces:**
- Produces HTTP API: `POST /ingest/events` (JSONL body → `events` table; idempotent via unique `(registry_key, offset)` + `INSERT OR IGNORE`, so redelivered batches are harmless; when `batch_start_offset` is AHEAD of the collector's max known offset for that `registry_key` — server-side data loss/reset — respond with a distinguishable gap status carrying the expected offset instead of 2xx, so the forwarder rewinds and backfills, never a silent hole), `POST /ingest/snapshot` (→ `snapshots` upsert-latest + history), `GET /fleet` (one row per registry_key: latest snapshot summary + last event + in-flight agents), `GET /loops/:key/events?since=<offset>` (same monotonic offset as ingest keys). Auth: `Authorization: Bearer <pre-shared key>` when `COLLECTOR_TOKEN` env set; without a token the listener binds LOOPBACK ONLY and refuses to start on a non-loopback interface (PR #80 review).
