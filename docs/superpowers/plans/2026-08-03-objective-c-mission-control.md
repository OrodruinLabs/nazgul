...
**Goal:** Ship the fleet substrate — global registry, full-event-bus forwarding, a collector service owning the database (SQLite local / Postgres remote), OTel identity stamping, `/nazgul:fleet` CLI view, and doctor checks — on top of Claude Code's first-party chrome: Agent View (background-session lifecycle: attach/logs/stop/respawn) and Remote Control (remote view/steer/notify of interactive sessions). Registry + files are primary for row existence and loop state; Agent View is progressive enhancement (2026-08-03 platform research).
...
- Offset contract (SHARED by Tasks 3/4/5 — CodeRabbit PR #82): all offsets are BYTE offsets into `events.jsonl` (UTF-8, one JSON record per `\n`-terminated line). A record's offset = the byte position of its first byte; `batch_start_offset` = the first record's offset; the collector derives each subsequent record's offset as previous offset + byte length of the previous line INCLUDING its newline. The cursor holds the offset of the next unsent byte (exclusive end of acknowledged data); `?since=<offset>` is INCLUSIVE (records with offset >= since). Gap response: HTTP 409, body `{"error":"gap","expected_offset":<N>}` where N is the collector's next expected byte offset (0 for an empty/reset store).
- `registry_key` contract (SHARED by Tasks 3/4/5): the worktree's absolute PHYSICAL path (symlinks resolved, e.g. `cd -P`/`realpath`), exactly as normalized by `registry_list` — one worktree, one key, no alias splits.
- Produces: POSTs to `<collector.url>/ingest/events` (JSONL batch since last cursor, cursor persisted at `nazgul/.collector-cursor`; each batch carries `registry_key` + `batch_start_offset` so redelivery is idempotent server-side — cursor advances ONLY after a 2xx, so a committed-then-timed-out response resends the same keyed batch harmlessly, PR #80 review) and `<collector.url>/ingest/snapshot` (body: `{registry_key, feat_id, snapshot: <checkpoint JSON>, in_flight: [...]}` with `prompt_head` fields REDACTED before send — they can contain prompt text) **[MOOT as of FEAT-034/ADR-028: `prompt_head` no longer exists. The marker carries `prompt_hash` (16 hex of sha256) and `prompt_bytes`, so no prompt text reaches a marker and there is nothing to redact. Historical text left as written.]**. Remote (non-loopback) `collector.url` must be `https://`; plain `http://` to a non-loopback host → refuse with one stderr warning, no send. Disabled/unreachable → silent no-op, cursor unmoved.
- Gap/resync handshake (2026-08-03 research — keeps the projection provably rebuildable from files): when the collector answers a batch with the gap status (Task 4 — its max known offset for this `registry_key` is BELOW `batch_start_offset`, i.e. the server lost or reset its data), the forwarder rewinds `nazgul/.collector-cursor` to the server's stated expected offset and resends from there on the next invocation. Without this, a wiped collector DB leaves every client cursor ahead of the server — a permanent, silent gap.

- [ ] **Step 1: Write failing tests**: (a) enabled + fake collector → events since cursor POSTed once with `batch_start_offset`, cursor advances only on 2xx; (b) collector down → exit 0, cursor unmoved; (c) `collector.enabled: false` → no request attempted (fixture asserts zero hits); (d) malformed cursor file → resend from start, warning on stderr; (e) snapshot payload contains no `prompt_head` values **[MOOT — FEAT-034/ADR-028 removed the field at the writer; this test would assert the absence of something that is never produced]**; (f) `http://` non-loopback URL → refused, zero requests, one stderr warning; (g) fake collector answers 409 `{"error":"gap","expected_offset":N}` → cursor rewound to N, stderr note; (h) offset-zero contract: fresh log + fresh collector → first batch carries `batch_start_offset: 0`; rewind to `expected_offset: 0` resends from the start of the file.
...
**Interfaces:**
- Produces HTTP API: `POST /ingest/events` (JSONL body → `events` table; per-record offsets derived per the Task 3 shared byte-offset contract; idempotent via unique `(registry_key, offset)` + `INSERT OR IGNORE`, so redelivered batches are harmless; when `batch_start_offset` is AHEAD of the collector's max known offset for that `registry_key` — server-side data loss/reset — respond per the Task 3 gap contract: HTTP 409 `{"error":"gap","expected_offset":<N>}` instead of 2xx, nothing inserted, so the forwarder rewinds and backfills, never a silent hole), `POST /ingest/snapshot` (→ `snapshots` upsert-latest + history), `GET /fleet` (one row per registry_key: latest snapshot summary + last event + in-flight agents), `GET /loops/:key/events?since=<offset>` (inclusive, same byte offsets as ingest keys). Auth: `Authorization: Bearer $COLLECTOR_TOKEN` (pre-shared, from the `COLLECTOR_TOKEN` env var) when set; without a token the listener binds LOOPBACK ONLY and refuses to start on a non-loopback interface (PR #80 review).
- Schema: `events(id, registry_key, offset, ts, event, payload_json, UNIQUE(registry_key, offset))`, `snapshots(registry_key, feat_id, iteration, ts, payload_json)`, indexes on `(registry_key, ts)`. All SQL through `db.js` (better-sqlite3 now; the adapter's contract is plain parameterized SQL so a `pg` implementation is a drop-in later — no ORM). Commit `collector/package-lock.json`; the CI job uses `actions/setup-node` (Node LTS, npm cache) + `npm ci`.

- [ ] **Step 1: Write failing ingest tests** (node:test + supertest-style fetch against an ephemeral server + `:memory:` DB): events batch lands with correct rows; SAME batch redelivered → no duplicate rows (idempotency); snapshot upserts latest; batch whose `batch_start_offset` is ahead of the max known offset → 409 gap body with `expected_offset`, nothing inserted; offset-zero contract: empty store accepts `batch_start_offset: 0`, empty store + `batch_start_offset > 0` → 409 with `expected_offset: 0`; non-empty replay: after a gap-triggered rewind, the resent overlapping range lands without duplicates and `GET /loops/:key/events?since=` (inclusive) returns it; bad JSON → 400 without crash and without partial batch insert (atomic); token required when env set; no token + non-loopback bind requested → server refuses to start. Before writing these tests, skim two references for schema/naming shape ONLY (2026-08-03 research): builderz-labs Mission Control's data model (MIT, alpha — read, never depend) and Managed Agents' SSE event taxonomy (`session.status_*` with `stop_reason`, `agent.tool_use`, `span.model_request_end` token usage) — adopt names only where the mapping to Nazgul's existing event types is obvious, so a future first-party fleet API is a translation layer, not a rewrite.
...
- Join semantics (2026-08-03 research): join on `cwd` per the documented `claude agents --json` contract (fields incl. `cwd`, `kind`, `state`, `pid`, `sessionId`, `waitingFor`) — no registration API is needed and none is assumed. Both sides of the join are normalized to the Task 3 `registry_key` form (absolute physical path, symlinks resolved) BEFORE comparison, so distinct worktrees never collapse into one row and a path alias never makes a live session render as `session: -` (CodeRabbit PR #82). Registry × files decide row EXISTENCE and loop state; Agent View contributes session liveness/PID only. A registry row with no matching Agent View session renders in full with `session: -` — this is NORMAL (foreground/interactive loops, remote sessions, other machines), so absence from Agent View is never treated as a death signal and never feeds attention ordering.

> **Amendment (2026-08-16, messaging-adoption cycle):** re-pin the row shape at pickup; the 2.1.233-measured shape differs from the list above (`state` vs `status`; `name` present; `pid` sometimes absent). See the platform-facts doc in specs/.

- [ ] **Step 1: Write failing tests**: (a) fake `claude` returns two sessions matching two registry worktrees → joined rows with session state; (b) fake `claude` absent → rows render from files + explicit degradation header (never empty, never silent); (c) fake `claude` returns changed/unparseable JSON → same as (b) with `contract-drift` wording; (d) BLOCKED loop sorts first; (e) registry entry with NO matching Agent View session → full row rendered with `session: -`, no attention flag from the absence itself.
...
- The private `nazgul.*` namespace on stable core OTel is DELIBERATE — not `gen_ai.*`: the gen_ai conventions were deprecated out of the main semconv repo at v1.42.0 (June 2026) and the dedicated genai repo has no tagged release and no pinnable schema URL (verified 2026-08-03). Do not "upgrade" this to `gen_ai.*` until that repo cuts a tagged release.
...
- [ ] **Step 1:** File the frontend as its own inbox item (`nazgul/inbox/mission-control-frontend.md`, priority after this objective) citing spec §5 C3 as amended 2026-08-03 (fleet page, drill-down rendering Nazgul's inner state, pause/resume via config writes, pre-shared-key auth; remote view/steer/notify defaults to first-party Remote Control — the xterm.js terminal panel and service-worker notifications are recorded FALLBACKS, built only if that spec cycle finds Remote Control unavailable or insufficient) and requiring a fresh brainstorm+spec+plan cycle at pickup (Agent View and Remote Control contracts will both have moved). Consumer-side OTel caveat for that cycle: `gen_ai.*` attribute names are unpinned upstream — match loosely, never validate against them.
