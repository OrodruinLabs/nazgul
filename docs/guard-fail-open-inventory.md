...
1. **`scripts/git-hooks/pre-merge-commit:77`** — `PARALLEL` defaults to `"false"` on a corrupt-but-
   ...
   unbounded false-allow cost. **FIXED (PR #74 review)** — `jq -e .` fail-closed gate added; was highest severity in this inventory.
...
expansions the grep's clauses don't match. Four were found by reading the 16 files in full, not by
the grep; the fifth is `parallel-dispatch-guard.sh`'s missing-`run_in_background` branch, which #205
reclassified from a fail-closed block into a deliberate fail-open after that pass. None are counted
in the **125/90/18/17 totals above**, keeping that count exactly reproducible.
...
| `scripts/parallel-dispatch-guard.sh:86-95` — `if [ "$BACKGROUND_TYPE" = "missing" ] && [ "$NESTED_CALLER" != "true" ]; then ... emit_event "dispatch_guard_background_unverifiable"` | The Agent payload carries no `run_in_background` field at all | (c) | RECLASSIFIED by #205 from fail-closed-block to **fail-open-with-named-event**. It used to `exit 2` on the theory that a main-session caller can always state `false` explicitly; on hosts whose Agent schema has no such field the value is unsupplyable, so that block disabled the entire review board rather than tightening it. The hook payload cannot distinguish "schema lacks the field" from "caller omitted it", so ADR-009 cost-weighing decides the direction for THIS guard: a false BLOCK costs the review discipline outright, while a false ALLOW is bounded by FEAT-024's empty-return detection and by this guard's other reviewer-route checks, which are unchanged (an explicit `run_in_background:true` and a named dispatch both still exit 2). Audited in-file and logged as `dispatch_guard_background_unverifiable` (fields `agent`, `caller`) — background-verifiability recorded as ABSENT, never assumed. |
