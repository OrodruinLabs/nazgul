...
### Added
- **Runtime-verification gate** (roadmap 2.1, start of Phase 2 "Verification & Observability"). The review gate's pre-checks now run `build_command` as a **hard gate** — previously it was read but never executed, so a task could pass review and reach DONE with code that doesn't build. A new opt-in `project.smoke_command` runs the built artifact as a short, self-terminating check (e.g. `node dist/index.js --version`, an import-smoke, a healthcheck). Pre-check order is test → lint → build → smoke (stop at first failure); build/smoke failures route through the existing IN_PROGRESS→BLOCKED retry path, and results are recorded to the task's `verification.md`. Discovery suggests a smoke command. Config schema 8→9.
