...
if command -v jq &>/dev/null; then
    ...
    # Each alias filtered to non-empty strings: jq's // only skips null/false,
    # so a "" preferred path must not mask a populated legacy alias.
    FILE_PATH=$(echo "$INPUT" | jq -r '(.tool_input.file_path | strings | select(length > 0)) // (.tool_result.file_path | strings | select(length > 0)) // (.file_path | strings | select(length > 0)) // (.toolResult.file_path | strings | select(length > 0)) // (.result.file_path | strings | select(length > 0)) // empty' 2>/dev/null || true)
