...
migrate_30_to_31() {
  ...
  # (incl. false) preserved. Same type-guard pattern as migrate_27_to_28.
  jq '
    .guards = ((if (.guards | type) == "object" then .guards else {} end)
...
migrate_32_to_33() {
  local tmp; tmp=$(mktemp)
  # MF-051: TASK-001 deleted the only consumer (stop-hook teardown gate +
  # tt_detect_undismissed()). guards.team_sweep* is a separate mechanism
  # (ADR-017 Option C) and is left untouched.
  jq '
    (if (.guards | type) == "object" then .guards else {} end) as $guards
    | (if (._deprecated_removed | type) == "object" then ._deprecated_removed else {} end) as $dep
    | ($dep
        + (if ($guards | has("team_teardown")) and ($guards.team_teardown != true)
           then {"guards.team_teardown": $guards.team_teardown} else {} end)
      ) as $newdep
    | (if ($newdep | length) > 0 then ._deprecated_removed = $newdep else . end)
    | .guards = ($guards | del(.team_teardown))
    | .schema_version = 33
  ' "$CONFIG" > "$tmp" && mv "$tmp" "$CONFIG"
