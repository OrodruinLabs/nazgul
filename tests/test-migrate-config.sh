...
assert_json_field "v1 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v2 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v3 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v4 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v5 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v6 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v7 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v8 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v12 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v13 → v25 (full chain) schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v14 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v16 → v25 schema_version (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v9 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v10 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v11 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "unversioned modern → reaches v24" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v15 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v16→v17 granularity task→group schema=21 (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v17 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v18 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v20 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v26 terminal → schema_version still 32" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v25 → v26 schema_version reaches 32 (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v24 → v26 schema_version reaches 32 (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v1→v29 chain reaches schema_version 27" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v19 → v25 schema_version (walks to terminal)" "$TMPDIR_V20/nazgul/config.json" ".schema_version" "32"
...
assert_json_field "v23→v32 schema_version is 32 (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v17→v27 walk reaches schema_version 24" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v24→v32 schema_version is 32 (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "32"
...
assert_json_field "v26 garbage conductor: schema_version reaches 32 (full chain)" "$CFG" ".schema_version" "32"
...
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v27: schema_version reaches 32 (full chain)" "$CFG" ".schema_version" "32"
...
# --- v31 -> v32: guards.subagent_resume added (additive, default true) ---
...
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v32: schema_version reaches 32" "$CFG" ".schema_version" "32"
assert_eq "v32: guards.subagent_resume defaults true" \
  "$(jq -r '.guards.subagent_resume' "$CFG")" "true"
assert_eq "v32: preserves existing guards.team_teardown" \
  "$(jq -r '.guards.team_teardown' "$CFG")" "true"
...
# --- v31 -> v32: config already at 32 is unchanged ---
NAZGUL_DIR=$(setup_nazgul_dir "v32-already-current")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 32, "guards": {"subagent_resume": false}}
EOF
BEFORE=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
AFTER=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "v32 already current: config unchanged" "$BEFORE" "$AFTER"
...
assert_json_field "template: schema_version is 32" "$TEMPLATE_FILE" ".schema_version" "32"
...
assert_json_field "no-positional-arg: fallback migrated the FIXTURE root's config, not cwd's" "$FALLBACK_ROOT/nazgul/config.json" ".schema_version" "32"
