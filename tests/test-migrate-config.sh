...
assert_json_field "v1 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v2 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v3 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v4 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v5 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v6 config → v8 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v7 → v8 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
assert_json_field "v8 → v9 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "9"
...
# --- Test 3j: v9 config → no-op ---
NAZGUL_DIR=$(setup_nazgul_dir "v9-config")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 9, "mode": "hitl" }
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_eq "v9 config → no output" "$OUTPUT" ""
