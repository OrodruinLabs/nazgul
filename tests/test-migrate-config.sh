...
assert_json_field "v1 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v2 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v3 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v4 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v5 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v6 config → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v7 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v8 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v12 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v13 → v25 (full chain) schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v14 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v16 → v25 schema_version (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v9 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v10 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v11 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "unversioned modern → reaches v24" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v15 → v25 schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v16→v17 granularity task→group schema=21 (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v17 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v18 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v20 → v25 schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_contains "v1→v27 chain migrated" "$OUTPUT" "migrated"
assert_json_field "v1→v27 chain reaches schema_version 27" "$NAZGUL_DIR/config.json" ".schema_version" "27"
assert_json_field "v1→v27 chain granularity is group" "$NAZGUL_DIR/config.json" ".review_gate.granularity" "group"
assert_json_field "v1→v27 chain post_loop is sonnet" "$NAZGUL_DIR/config.json" ".models.post_loop" "sonnet"
assert_json_field "v1→v27 chain wave_execution is true" "$NAZGUL_DIR/config.json" ".parallelism.wave_execution" "true"
assert_json_field "v1→v27 chain docs.verify_post_loop is true" "$NAZGUL_DIR/config.json" ".docs.verify_post_loop" "true"
assert_json_field "v1→v27 chain review_gate.require_provenance is true" "$NAZGUL_DIR/config.json" ".review_gate.require_provenance" "true"
assert_json_field "v1→v27 chain review_gate.conditional_dispatch is false" "$NAZGUL_DIR/config.json" ".review_gate.conditional_dispatch" "false"
assert_json_field "v1→v27 chain docs.verify_comments is true" "$NAZGUL_DIR/config.json" ".docs.verify_comments" "true"
assert_json_field "v1→v27 chain models.review is haiku" "$NAZGUL_DIR/config.json" ".models.review" "haiku"
assert_json_field "v1→v27 chain models.review_by_reviewer security-reviewer is sonnet" "$NAZGUL_DIR/config.json" '.models.review_by_reviewer["security-reviewer"]' "sonnet"
assert_json_field "v1→v23→v27 chain execution.engine sequential → execution.parallel=false" "$NAZGUL_DIR/config.json" ".execution.parallel" "false"
assert_json_field "v1→v23→v27 chain conductor.gates.approve_graph false → execution.gates.approve_plan" "$NAZGUL_DIR/config.json" ".execution.gates.approve_plan" "false"
assert_json_field "v1→v23→v27 chain conductor.max_parallel 3 → execution.max_parallel" "$NAZGUL_DIR/config.json" ".execution.max_parallel" "3"
assert_json_field "v1→v23→v27 chain conductor.enforce.dispatch_guard true → execution.enforce.dispatch_guard" "$NAZGUL_DIR/config.json" ".execution.enforce.dispatch_guard" "true"
assert_json_field "v1→v27 chain automation.heartbeat.enabled is false" "$NAZGUL_DIR/config.json" ".automation.heartbeat.enabled" "false"
assert_json_field "v1→v23→v27 chain auto_start.engine conductor → auto_start.parallel=true" "$NAZGUL_DIR/config.json" ".automation.heartbeat.auto_start.parallel" "true"
assert_json_field "v1→v23→v27 chain models.conductor created then deleted by v26" "$NAZGUL_DIR/config.json" '.models | has("conductor")' "false"
assert_json_field "v1→v27 chain models.review_orchestrator seeded from review (haiku)" "$NAZGUL_DIR/config.json" ".models.review_orchestrator" "haiku"
assert_json_field "v1→v27 chain models.review_default seeded from review (haiku)" "$NAZGUL_DIR/config.json" ".models.review_default" "haiku"
assert_json_field "v1→v27 chain self_audit.enabled is true" "$NAZGUL_DIR/config.json" ".self_audit.enabled" "true"
assert_json_field "v1→v27 chain self_audit.backlog_path" "$NAZGUL_DIR/config.json" ".self_audit.backlog_path" "nazgul/improvements.md"
assert_json_field "v1→v23→v27 chain conductor.enforce.premerge_guard true → execution.enforce.premerge_guard" "$NAZGUL_DIR/config.json" ".execution.enforce.premerge_guard" "true"
assert_json_field "v1→v27 chain branch.prior_hooks_path is null" "$NAZGUL_DIR/config.json" ".branch.prior_hooks_path" "null"
assert_json_field "v1→v27 chain guards.git_hooks is true" "$NAZGUL_DIR/config.json" ".guards.git_hooks" "true"
assert_json_field "v1→v27 chain review_gate.unverified_retries is 2" "$NAZGUL_DIR/config.json" ".review_gate.unverified_retries" "2"
assert_json_field "v1→v27 chain review_gate.allow_unverified_nonblocking is true" "$NAZGUL_DIR/config.json" ".review_gate.allow_unverified_nonblocking" "true"
assert_json_field "v1→v27 chain review_gate.critical_reviewers[0]" "$NAZGUL_DIR/config.json" ".review_gate.critical_reviewers[0]" "security-reviewer"
assert_json_field "v1→v27 chain review_gate.adversarial_crosscheck is true" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_crosscheck" "true"
assert_json_field "v1→v27 chain review_gate.adversarial_margin is 10" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_margin" "10"
assert_json_field "v1→v27 chain review_gate.adversarial_max is 3" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_max" "3"
assert_json_field "v1→v27 chain connectors.github.enabled is false" "$NAZGUL_DIR/config.json" ".connectors.github.enabled" "false"
assert_json_field "v1→v27 chain connectors.github.pull.label is nazgul" "$NAZGUL_DIR/config.json" ".connectors.github.pull.label" "nazgul"
assert_json_field "v1→v27 chain connectors.github.pull.claimed_label is nazgul-claimed" "$NAZGUL_DIR/config.json" ".connectors.github.pull.claimed_label" "nazgul-claimed"
assert_json_field "v1→v27 chain connectors.github.pull.max_body_bytes is 65536" "$NAZGUL_DIR/config.json" ".connectors.github.pull.max_body_bytes" "65536"
assert_json_field "v1→v27 chain connectors.github.push.enabled is true" "$NAZGUL_DIR/config.json" ".connectors.github.push.enabled" "true"
assert_json_field "v1→v27 chain connectors.github.pull_failures is 0" "$NAZGUL_DIR/config.json" ".connectors.github.pull_failures" "0"
assert_json_field "v1→v27 chain connectors.github.map is empty object" "$NAZGUL_DIR/config.json" ".connectors.github.map | length" "0"
...
assert_json_field "v19 → v25 schema_version (walks to terminal)" "$TMPDIR_V20/nazgul/config.json" ".schema_version" "27"
...
assert_json_field "v23→v27 schema_version is 24" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v17→v27 walk reaches schema_version 24" "$NAZGUL_DIR/config.json" ".schema_version" "27"
...
assert_json_field "v24→v27 schema_version is 25" "$NAZGUL_DIR/config.json" ".schema_version" "27"
