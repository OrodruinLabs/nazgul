...
# The one literal pin for the terminal schema. Every walk assertion below reads it,
# so a new migration step updates this line and templates/config.json, not 50 assertions.
EXPECTED_TERMINAL=37
...
assert_json_field "v1 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v2 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v3 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v4 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v5 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v6 config → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v7 → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v8 → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v12 → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v13 → terminal (full chain) schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v14 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v16 → terminal schema_version (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v9 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v10 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_json_field "v11 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "unversioned modern → reaches terminal $EXPECTED_TERMINAL" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v15 → terminal schema_version (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v16→v17 granularity task→group, schema reaches terminal (via v17)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v17 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v18 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v20 → terminal schema_version" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v26 terminal → schema_version reaches terminal $EXPECTED_TERMINAL" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v25 → terminal schema_version reaches terminal $EXPECTED_TERMINAL (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v24 → terminal schema_version reaches terminal $EXPECTED_TERMINAL (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
assert_contains "v1→v29 chain migrated" "$OUTPUT" "migrated"
assert_json_field "v1→terminal chain reaches schema_version $EXPECTED_TERMINAL" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
assert_json_field "v1→v29 chain granularity is group" "$NAZGUL_DIR/config.json" ".review_gate.granularity" "group"
assert_json_field "v1→v29 chain post_loop is sonnet" "$NAZGUL_DIR/config.json" ".models.post_loop" "sonnet"
assert_json_field "v1→v29 chain wave_execution is true" "$NAZGUL_DIR/config.json" ".parallelism.wave_execution" "true"
assert_json_field "v1→v29 chain docs.verify_post_loop is true" "$NAZGUL_DIR/config.json" ".docs.verify_post_loop" "true"
assert_json_field "v1→v29 chain review_gate.require_provenance is true" "$NAZGUL_DIR/config.json" ".review_gate.require_provenance" "true"
assert_json_field "v1→v29 chain review_gate.conditional_dispatch is false" "$NAZGUL_DIR/config.json" ".review_gate.conditional_dispatch" "false"
assert_json_field "v1→v29 chain docs.verify_comments is true" "$NAZGUL_DIR/config.json" ".docs.verify_comments" "true"
assert_json_field "v1→v29 chain models.review is haiku" "$NAZGUL_DIR/config.json" ".models.review" "haiku"
assert_json_field "v1→v29 chain models.review_by_reviewer security-reviewer is sonnet" "$NAZGUL_DIR/config.json" '.models.review_by_reviewer["security-reviewer"]' "sonnet"
assert_json_field "v1→v23→v29 chain execution.engine sequential → execution.parallel=false" "$NAZGUL_DIR/config.json" ".execution.parallel" "false"
assert_json_field "v1→v23→v29 chain conductor.gates.approve_graph false → execution.gates.approve_plan" "$NAZGUL_DIR/config.json" ".execution.gates.approve_plan" "false"
assert_json_field "v1→v23→v29 chain conductor.max_parallel 3 → execution.max_parallel" "$NAZGUL_DIR/config.json" ".execution.max_parallel" "3"
assert_json_field "v1→v23→v29 chain conductor.enforce.dispatch_guard true → execution.enforce.dispatch_guard" "$NAZGUL_DIR/config.json" ".execution.enforce.dispatch_guard" "true"
assert_json_field "v1→v29 chain automation.heartbeat.enabled is false" "$NAZGUL_DIR/config.json" ".automation.heartbeat.enabled" "false"
assert_json_field "v1→v23→v29 chain auto_start.engine conductor → auto_start.parallel=true" "$NAZGUL_DIR/config.json" ".automation.heartbeat.auto_start.parallel" "true"
assert_json_field "v1→v23→v29 chain models.conductor created then deleted by v26" "$NAZGUL_DIR/config.json" '.models | has("conductor")' "false"
assert_json_field "v1→v29 chain models.review_orchestrator seeded from review (haiku)" "$NAZGUL_DIR/config.json" ".models.review_orchestrator" "haiku"
assert_json_field "v1→v29 chain models.review_default seeded from review (haiku)" "$NAZGUL_DIR/config.json" ".models.review_default" "haiku"
assert_json_field "v1→v29 chain self_audit.enabled is true" "$NAZGUL_DIR/config.json" ".self_audit.enabled" "true"
assert_json_field "v1→v29 chain self_audit.backlog_path" "$NAZGUL_DIR/config.json" ".self_audit.backlog_path" "nazgul/improvements.md"
assert_json_field "v1→v23→v29 chain conductor.enforce.premerge_guard true → execution.enforce.premerge_guard" "$NAZGUL_DIR/config.json" ".execution.enforce.premerge_guard" "true"
assert_json_field "v1→v29 chain branch.prior_hooks_path is null" "$NAZGUL_DIR/config.json" ".branch.prior_hooks_path" "null"
assert_json_field "v1→v29 chain guards.git_hooks is true" "$NAZGUL_DIR/config.json" ".guards.git_hooks" "true"
assert_json_field "v1→v29 chain review_gate.unverified_retries is 2" "$NAZGUL_DIR/config.json" ".review_gate.unverified_retries" "2"
assert_json_field "v1→v29 chain review_gate.allow_unverified_nonblocking is true" "$NAZGUL_DIR/config.json" ".review_gate.allow_unverified_nonblocking" "true"
assert_json_field "v1→v29 chain review_gate.critical_reviewers[0]" "$NAZGUL_DIR/config.json" ".review_gate.critical_reviewers[0]" "security-reviewer"
assert_json_field "v1→v29 chain review_gate.adversarial_crosscheck is true" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_crosscheck" "true"
assert_json_field "v1→v29 chain review_gate.adversarial_margin is 10" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_margin" "10"
assert_json_field "v1→v29 chain review_gate.adversarial_max is 3" "$NAZGUL_DIR/config.json" ".review_gate.adversarial_max" "3"
assert_json_field "v1→v29 chain connectors.github.enabled is false" "$NAZGUL_DIR/config.json" ".connectors.github.enabled" "false"
assert_json_field "v1→v29 chain connectors.github.pull.label is nazgul" "$NAZGUL_DIR/config.json" ".connectors.github.pull.label" "nazgul"
assert_json_field "v1→v29 chain connectors.github.pull.claimed_label is nazgul-claimed" "$NAZGUL_DIR/config.json" ".connectors.github.pull.claimed_label" "nazgul-claimed"
assert_json_field "v1→v29 chain connectors.github.pull.max_body_bytes is 65536" "$NAZGUL_DIR/config.json" ".connectors.github.pull.max_body_bytes" "65536"
assert_json_field "v1→v29 chain connectors.github.push.enabled is true" "$NAZGUL_DIR/config.json" ".connectors.github.push.enabled" "true"
assert_json_field "v1→v29 chain connectors.github.pull_failures is 0" "$NAZGUL_DIR/config.json" ".connectors.github.pull_failures" "0"
assert_json_field "v1→v29 chain connectors.github.map is empty object" "$NAZGUL_DIR/config.json" ".connectors.github.map | length" "0"
...
assert_json_field "v19 → terminal schema_version (walks to terminal)" "$TMPDIR_V20/nazgul/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v23→terminal schema_version is $EXPECTED_TERMINAL (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v17→terminal walk reaches schema_version $EXPECTED_TERMINAL" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v24→terminal schema_version is $EXPECTED_TERMINAL (full chain)" "$NAZGUL_DIR/config.json" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_json_field "v26 garbage conductor: schema_version reaches terminal $EXPECTED_TERMINAL (full chain)" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
...
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v27: schema_version reaches terminal $EXPECTED_TERMINAL (full chain)" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
...
# --- v27 -> v28: bash_write_reconciliation + lock_stale_seconds added (additive, defaults true/300) ---
NAZGUL_DIR=$(setup_nazgul_dir "v27-to-28")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 27, "guards": {"lean_comments": true}, "automation": {"heartbeat": {"enabled": true}}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v28 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_eq "v28: automation.heartbeat.lock_stale_seconds defaults 300" \
  "$(jq -r '.automation.heartbeat.lock_stale_seconds' "$CFG")" "300"
assert_eq "v28: preserves existing guards.lean_comments" \
  "$(jq -r '.guards.lean_comments' "$CFG")" "true"
assert_eq "v28: preserves existing automation.heartbeat.enabled" \
  "$(jq -r '.automation.heartbeat.enabled' "$CFG")" "true"

# --- v27 -> v28: explicit non-default values preserved ---
NAZGUL_DIR=$(setup_nazgul_dir "v27-to-28-explicit")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 27, "guards": {"bash_write_reconciliation": false}, "automation": {"heartbeat": {"lock_stale_seconds": 900}}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v28: explicit bash_write_reconciliation false preserved" \
  "$(jq -r '.guards.bash_write_reconciliation' "$CFG")" "false"
assert_eq "v28: explicit lock_stale_seconds 900 preserved" \
  "$(jq -r '.automation.heartbeat.lock_stale_seconds' "$CFG")" "900"

# --- v27 -> v28: non-object guards/automation/heartbeat clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v27-to-28-garbage")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 27, "guards": "garbage", "automation": {"heartbeat": "garbage"}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v28 garbage guards/heartbeat: migrator exits 0" "$MIG_EC" 0
assert_eq "v28 garbage guards: bash_write_reconciliation defaults true" \
  "$(jq -r '.guards.bash_write_reconciliation' "$CFG")" "true"
assert_eq "v28 garbage heartbeat: lock_stale_seconds defaults 300" \
  "$(jq -r '.automation.heartbeat.lock_stale_seconds' "$CFG")" "300"

# --- v28 -> v29: review_gate.stall_retry_escalate_tier added (additive, default true) ---
NAZGUL_DIR=$(setup_nazgul_dir "v28-to-29")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 28, "review_gate": {"granularity": "group"}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v29 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
...
# --- v28 -> v29: explicit false value preserved ---
NAZGUL_DIR=$(setup_nazgul_dir "v28-to-29-explicit")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 28, "review_gate": {"stall_retry_escalate_tier": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v29: explicit stall_retry_escalate_tier false preserved" \
  "$(jq -r '.review_gate.stall_retry_escalate_tier' "$CFG")" "false"

# --- v28 -> v29: non-object review_gate clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v28-to-29-garbage")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 28, "review_gate": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v29 garbage review_gate: migrator exits 0" "$MIG_EC" 0
...
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v29->v30 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
...
assert_eq "review_orchestrator is NOT re-touched by migrate_29_to_30 (already sonnet)" \
  "$(jq -r '.models.review_orchestrator' "$CFG")" "sonnet"

NAZGUL_DIR=$(setup_nazgul_dir "v29-to-30-explicit-false")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "review_gate": {"receipt_hash_enforcement": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "review_gate": {"receipt_hash_enforcement": true}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "models": {"review_orchestrator": "haiku"}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "review_orchestrator is NOT re-touched by migrate_29_to_30 (custom non-default value)" \
  "$(jq -r '.models.review_orchestrator' "$CFG")" "haiku"

# --- non-object review_gate clamps instead of erroring (migrate_29_to_30) ---
NAZGUL_DIR=$(setup_nazgul_dir "v29-to-30-garbage-review-gate")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "review_gate": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "safety": {"block_destructive_commands": true, "require_tests_pass_before_review": true, "max_consecutive_failures": 5, "consecutive_failures": 2}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "dead safety keys removed on migration (block_destructive_commands)" \
  "$(jq -r '.safety | has("block_destructive_commands")' "$CFG")" "false"
assert_eq "dead safety keys removed on migration (require_tests_pass_before_review)" \
  "$(jq -r '.safety | has("require_tests_pass_before_review")' "$CFG")" "false"
assert_eq "dead safety key removal preserves live safety.max_consecutive_failures" \
  "$(jq -r '.safety.max_consecutive_failures' "$CFG")" "5"
assert_eq "dead safety key removal preserves live safety.consecutive_failures" \
  "$(jq -r '.safety.consecutive_failures' "$CFG")" "2"
assert_eq "dead safety keys at old default leave no ._deprecated_removed entry" \
  "$(jq -r 'has("_deprecated_removed")' "$CFG")" "false"

# --- MF-051: dead top-level keys removed on migration (values still at old default) ---
NAZGUL_DIR=$(setup_nazgul_dir "v29-to-30-dead-toplevel-keys-default")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "task_file": "nazgul/plan.md", "log_dir": "nazgul/logs", "review_dir": "nazgul/reviews"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "dead top-level keys removed on migration (task_file)" "$(jq -r 'has("task_file")' "$CFG")" "false"
assert_eq "dead top-level keys removed on migration (log_dir)" "$(jq -r 'has("log_dir")' "$CFG")" "false"
assert_eq "dead top-level keys removed on migration (review_dir)" "$(jq -r 'has("review_dir")' "$CFG")" "false"
assert_eq "dead top-level keys at old default leave no ._deprecated_removed entry" \
  "$(jq -r 'has("_deprecated_removed")' "$CFG")" "false"

# --- MF-051: customized dead-key values are preserved under ._deprecated_removed, not silently dropped ---
NAZGUL_DIR=$(setup_nazgul_dir "v29-to-30-dead-keys-customized")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "task_file": "custom/plan.md", "log_dir": "nazgul/logs", "review_dir": "custom/reviews", "safety": {"block_destructive_commands": false, "require_tests_pass_before_review": true}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "customized task_file value preserved under _deprecated_removed, not silently dropped" \
  "$(jq -r '._deprecated_removed.task_file' "$CFG")" "custom/plan.md"
assert_eq "customized review_dir value preserved under _deprecated_removed, not silently dropped" \
  "$(jq -r '._deprecated_removed.review_dir' "$CFG")" "custom/reviews"
assert_eq "log_dir at old default is NOT preserved under _deprecated_removed (carries no information)" \
  "$(jq -r '._deprecated_removed | has("log_dir")' "$CFG")" "false"
assert_eq "customized safety.block_destructive_commands=false preserved under _deprecated_removed" \
  "$(jq -r '._deprecated_removed."safety.block_destructive_commands"' "$CFG")" "false"
assert_eq "safety.require_tests_pass_before_review at old default is NOT preserved (carries no information)" \
  "$(jq -r '._deprecated_removed | has("safety.require_tests_pass_before_review")' "$CFG")" "false"
assert_eq "dead keys still fully removed even when a customized value is preserved (task_file)" \
  "$(jq -r 'has("task_file")' "$CFG")" "false"

# --- MF-051: non-object safety clamps instead of erroring (migrate_29_to_30) ---
NAZGUL_DIR=$(setup_nazgul_dir "v29-to-30-garbage-safety")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "safety": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v29->v30 garbage safety: migrator exits 0" "$MIG_EC" 0
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 29, "parallelism": {"enabled": false, "custom_flag": "kept"}, "context": {"budget_strategy": "conservative"}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "parallelism.* cluster is retained (not removed) by migrate_29_to_30" \
  "$(jq -r 'has("parallelism")' "$CFG")" "true"
assert_eq "parallelism.* values pass through migration completely untouched" \
  "$(jq -r '.parallelism.custom_flag' "$CFG")" "kept"
assert_eq "context.* cluster is retained (not removed) by migrate_29_to_30" \
  "$(jq -r 'has("context")' "$CFG")" "true"
assert_eq "context.* values pass through migration completely untouched" \
  "$(jq -r '.context.budget_strategy' "$CFG")" "conservative"

# --- v31 -> v33: guards.subagent_resume added at v32 (default true); guards.team_teardown
# (default true here) is then removed at v33 with no ._deprecated_removed entry ---
NAZGUL_DIR=$(setup_nazgul_dir "v31-to-32")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 31, "guards": {"team_teardown": true}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v32→v33 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
assert_eq "v32: guards.subagent_resume defaults true" \
  "$(jq -r '.guards.subagent_resume' "$CFG")" "true"
assert_eq "v33: guards.team_teardown removed (was at default true)" \
  "$(jq -r '.guards | has("team_teardown")' "$CFG")" "false"
assert_eq "v33: default-true team_teardown leaves no ._deprecated_removed entry" \
  "$(jq -r 'has("_deprecated_removed")' "$CFG")" "false"

# --- v31 -> v32: explicit false value preserved ---
NAZGUL_DIR=$(setup_nazgul_dir "v31-to-32-explicit-false")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 31, "guards": {"subagent_resume": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v32: explicit guards.subagent_resume=false preserved" \
  "$(jq -r '.guards.subagent_resume' "$CFG")" "false"

# --- v31 -> v32: non-object guards clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v31-to-32-garbage-guards")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 31, "guards": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v32 garbage guards: migrator exits 0" "$MIG_EC" 0
assert_eq "v32 garbage guards: subagent_resume defaults true" \
  "$(jq -r '.guards.subagent_resume' "$CFG")" "true"

# --- config already at terminal version is unchanged ---
NAZGUL_DIR=$(setup_nazgul_dir "terminal-already-current")
cat > "$NAZGUL_DIR/config.json" << EOF
{"schema_version": $EXPECTED_TERMINAL, "guards": {"subagent_resume": false}}
EOF
BEFORE=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
AFTER=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "already at terminal v$EXPECTED_TERMINAL: config unchanged" "$BEFORE" "$AFTER"

# --- migrate_31_to_32: full idempotency — run twice yields same output ---
NAZGUL_DIR=$(setup_nazgul_dir "v31-to-32-idempotent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 31, "guards": { "subagent_resume": false } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
...
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
SECOND=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "v31→v32 full idempotency (run twice = run once)" "$FIRST" "$SECOND"

# --- MF-051: v32 -> v33 removes guards.team_teardown; a customized non-default
# value survives under ._deprecated_removed (ADR-017 Consequences bullet 4) ---
NAZGUL_DIR=$(setup_nazgul_dir "v32-to-33-team-teardown-customized")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 32, "guards": {"team_teardown": false, "team_sweep": false, "team_sweep_min_age_hours": 12}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v33: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
assert_eq "v33: guards.team_teardown removed" "$(jq -r '.guards | has("team_teardown")' "$CFG")" "false"
assert_eq "v33: customized guards.team_teardown=false preserved under ._deprecated_removed" \
  "$(jq -r '._deprecated_removed."guards.team_teardown"' "$CFG")" "false"
assert_eq "v33: guards.team_sweep byte-identical (untouched sweep kill-switch)" \
  "$(jq -r '.guards.team_sweep' "$CFG")" "false"
assert_eq "v33: guards.team_sweep_min_age_hours byte-identical (untouched)" \
  "$(jq -r '.guards.team_sweep_min_age_hours' "$CFG")" "12"

# --- MF-051: v32 -> v33, guards.team_teardown at default true or absent leaves no
# ._deprecated_removed entry ---
NAZGUL_DIR=$(setup_nazgul_dir "v32-to-33-team-teardown-default")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 32, "guards": {"team_teardown": true, "team_sweep": true, "team_sweep_min_age_hours": 24}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v33: guards.team_teardown removed (was at default true)" \
...
assert_eq "v33: default-true team_teardown leaves no ._deprecated_removed entry" \
  "$(jq -r 'has("_deprecated_removed")' "$CFG")" "false"
assert_eq "v33: guards.team_sweep byte-identical" "$(jq -r '.guards.team_sweep' "$CFG")" "true"
assert_eq "v33: guards.team_sweep_min_age_hours byte-identical" \
  "$(jq -r '.guards.team_sweep_min_age_hours' "$CFG")" "24"

NAZGUL_DIR=$(setup_nazgul_dir "v32-to-33-team-teardown-absent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 32, "guards": {"team_sweep": true}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v33: absent guards.team_teardown stays absent" \
  "$(jq -r '.guards | has("team_teardown")' "$CFG")" "false"
assert_eq "v33: absent team_teardown leaves no ._deprecated_removed entry" \
  "$(jq -r 'has("_deprecated_removed")' "$CFG")" "false"

# --- v32 -> v33: non-object guards clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v32-to-33-garbage-guards")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 32, "guards": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v33 garbage guards: migrator exits 0" "$MIG_EC" 0
assert_eq "v33 garbage guards: team_teardown absent, guards clamped to object" \
  "$(jq -r '.guards | has("team_teardown")' "$CFG")" "false"

# --- migrate_32_to_33: full idempotency — run twice yields same output ---
NAZGUL_DIR=$(setup_nazgul_dir "v32-to-33-idempotent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 32, "guards": { "team_teardown": false } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
FIRST=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
SECOND=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "v32→v33 full idempotency (run twice = run once)" "$FIRST" "$SECOND"

# --- v33 -> v34: guards.in_flight_hold/in_flight_stale_minutes added with
# defaults; v33 outcomes (team_teardown removal, subagent_resume) untouched
# (TASK-008 stop-hook in-flight awareness kill-switch pair). Same type-guard
# pattern as migrate_30_to_31/migrate_31_to_32. ---
NAZGUL_DIR=$(setup_nazgul_dir "v33-to-34-defaults")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 33, "guards": {"subagent_resume": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v33→v34 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
assert_eq "v34: guards.in_flight_hold defaults true" \
  "$(jq -r '.guards.in_flight_hold' "$CFG")" "true"
assert_eq "v34: guards.in_flight_stale_minutes defaults 30" \
  "$(jq -r '.guards.in_flight_stale_minutes' "$CFG")" "30"
assert_eq "v34: v33 outcome guards.subagent_resume=false untouched" \
  "$(jq -r '.guards.subagent_resume' "$CFG")" "false"
assert_eq "v34: guards.team_teardown stays absent (v33 removal untouched)" \
  "$(jq -r '.guards | has("team_teardown")' "$CFG")" "false"

# --- v33 -> v34: explicit values (incl. false) preserved ---
NAZGUL_DIR=$(setup_nazgul_dir "v33-to-34-explicit")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 33, "guards": {"in_flight_hold": false, "in_flight_stale_minutes": 5}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v34: explicit guards.in_flight_hold=false preserved" \
  "$(jq -r '.guards.in_flight_hold' "$CFG")" "false"
assert_eq "v34: explicit guards.in_flight_stale_minutes=5 preserved" \
  "$(jq -r '.guards.in_flight_stale_minutes' "$CFG")" "5"

# --- v33 -> v34: non-object guards clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v33-to-34-garbage-guards")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 33, "guards": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v34 garbage guards: migrator exits 0" "$MIG_EC" 0
assert_eq "v34 garbage guards: in_flight_hold defaults true" \
  "$(jq -r '.guards.in_flight_hold' "$CFG")" "true"

# --- migrate_33_to_34: full idempotency — run twice yields same output ---
NAZGUL_DIR=$(setup_nazgul_dir "v33-to-34-idempotent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 33, "guards": { "in_flight_hold": false } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
FIRST=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
SECOND=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "v33→v34 full idempotency (run twice = run once)" "$FIRST" "$SECOND"

# --- v34 -> v35: execution.stacking.{enabled,max_unmerged,rework_priority} +
# top-level stack.layers added with defaults (opt-in stacked-PR continuation
# policy, FEAT-027 TASK-002). Same type-guard pattern as migrate_33_to_34. ---
NAZGUL_DIR=$(setup_nazgul_dir "v34-to-35-defaults")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 34, "execution": {"parallel": true}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "v34→v35 chain: schema_version reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
assert_eq "v35: execution.stacking.enabled defaults false" \
  "$(jq -r '.execution.stacking.enabled' "$CFG")" "false"
assert_eq "v35: execution.stacking.max_unmerged defaults 3" \
  "$(jq -r '.execution.stacking.max_unmerged' "$CFG")" "3"
assert_eq "v35: execution.stacking.rework_priority defaults 1" \
  "$(jq -r '.execution.stacking.rework_priority' "$CFG")" "1"
assert_eq "v35: stack.layers defaults to empty array" \
  "$(jq -r '.stack.layers | length' "$CFG")" "0"
assert_eq "v35: v34 outcome execution.parallel=true untouched" \
  "$(jq -r '.execution.parallel' "$CFG")" "true"

# --- v34 -> v35: explicit values (incl. false) preserved ---
NAZGUL_DIR=$(setup_nazgul_dir "v34-to-35-explicit")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 34, "execution": {"stacking": {"enabled": false, "max_unmerged": 5, "rework_priority": 2}}, "stack": {"layers": [{"feat_id": "FEAT-EXISTING"}]}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v35: explicit execution.stacking.enabled=false preserved" \
  "$(jq -r '.execution.stacking.enabled' "$CFG")" "false"
assert_eq "v35: explicit execution.stacking.max_unmerged=5 preserved" \
  "$(jq -r '.execution.stacking.max_unmerged' "$CFG")" "5"
assert_eq "v35: explicit execution.stacking.rework_priority=2 preserved" \
  "$(jq -r '.execution.stacking.rework_priority' "$CFG")" "2"
assert_eq "v35: explicit stack.layers preserved untouched" \
  "$(jq -r '.stack.layers[0].feat_id' "$CFG")" "FEAT-EXISTING"

# --- v34 -> v35: non-object execution/stack clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "v34-to-35-garbage")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 34, "execution": "garbage", "stack": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "v35 garbage execution/stack: migrator exits 0" "$MIG_EC" 0
assert_eq "v35 garbage execution: stacking.enabled defaults false" \
  "$(jq -r '.execution.stacking.enabled' "$CFG")" "false"
assert_eq "v35 garbage stack: layers defaults to empty array" \
  "$(jq -r '.stack.layers | length' "$CFG")" "0"

# --- migrate_34_to_35: full idempotency — run twice yields same output ---
NAZGUL_DIR=$(setup_nazgul_dir "v34-to-35-idempotent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 34, "execution": { "stacking": { "enabled": true } } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
FIRST=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
SECOND=$(jq -c '.' "$NAZGUL_DIR/config.json")
assert_eq "v34→v35 full idempotency (run twice = run once)" "$FIRST" "$SECOND"

# --- v35 -> v36: guards.red_run_evidence added with its default (kill switch for
# the IMPLEMENTED red-run evidence gate, FEAT-028 TASK-004). Same type-guard
# pattern as migrate_33_to_34/migrate_34_to_35. ---
NAZGUL_DIR=$(setup_nazgul_dir "red-run-kill-switch-defaults")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 35, "guards": {"in_flight_hold": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_json_field "red-run kill switch: chain reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
assert_eq "red-run kill switch: absent guards.red_run_evidence defaults true (default-on — this is an enforcement gate)" \
  "$(jq -r '.guards.red_run_evidence' "$CFG")" "true"
assert_eq "red-run kill switch: unrelated guards.in_flight_hold=false untouched" \
  "$(jq -r '.guards.in_flight_hold' "$CFG")" "false"
assert_eq "red-run kill switch: the walk adds only what its steps declare (.guards at v36, .project at v37)" \
  "$(jq -r 'keys | join(",")' "$CFG")" "guards,project,schema_version"

# --- v35 -> v36: an explicit value (including the opt-out `false`) is preserved,
# never reset to the default ---
NAZGUL_DIR=$(setup_nazgul_dir "red-run-kill-switch-explicit-false")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 35, "guards": {"red_run_evidence": false}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "red-run kill switch: explicit opt-out false preserved" \
  "$(jq -r '.guards.red_run_evidence' "$CFG")" "false"
assert_json_field "red-run kill switch: explicit opt-out still reaches terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"

# --- v35 -> v36: a non-object guards parent clamps instead of erroring ---
NAZGUL_DIR=$(setup_nazgul_dir "red-run-kill-switch-garbage-guards")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 35, "guards": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
assert_exit_code "red-run kill switch, garbage guards: migrator exits 0" "$MIG_EC" 0
assert_eq "red-run kill switch, garbage guards: red_run_evidence defaults true" \
  "$(jq -r '.guards.red_run_evidence' "$CFG")" "true"

# --- migrate_35_to_36: full idempotency — run twice yields same output ---
NAZGUL_DIR=$(setup_nazgul_dir "red-run-kill-switch-idempotent")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 35, "guards": { "red_run_evidence": false } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
...
# --- v36 -> v37: operator values for either key survive untouched (never clobbered) ---
NAZGUL_DIR=$(setup_nazgul_dir "tests-root-and-filter-template-operator-values")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 36, "project": {"test_roots": ["src/Api/tests", "src/Worker/tests"], "test_filter_template": "-k {filter}"}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
assert_eq "v36→v37: operator project.test_roots preserved (monorepo, two roots)" \
  "$(jq -c '.project.test_roots' "$CFG")" '["src/Api/tests","src/Worker/tests"]'
assert_eq "v36→v37: operator project.test_filter_template preserved (pytest form)" \
  "$(jq -r '.project.test_filter_template' "$CFG")" '-k {filter}'
assert_json_field "v36→v37: operator values still reach terminal $EXPECTED_TERMINAL" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"

# --- v36 -> v37: one key set, the other absent — the absent one is backfilled and the
# set one is left alone (the two keys are independent) ---
NAZGUL_DIR=$(setup_nazgul_dir "tests-root-and-filter-template-partial")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 36, "project": {"test_roots": ["test"]}}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
CFG="$NAZGUL_DIR/config.json"
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 36, "project": "garbage"}
EOF
OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>&1); MIG_EC=$?
CFG="$NAZGUL_DIR/config.json"
...
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{ "schema_version": 36, "project": { "test_filter_template": "-run {filter}" } }
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
FIRST=$(jq -c '.' "$NAZGUL_DIR/config.json")
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
...
V37_SCANNED=0; V37_CHECKED=0; V37_UNREADABLE=0; V37_FINDINGS=0
NAZGUL_DIR=$(setup_nazgul_dir "v37-surface-scan")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 36}
EOF
CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" >/dev/null 2>/dev/null
for surface in "$NAZGUL_DIR/config.json" "$REPO_ROOT/templates/config.json"; do
  for key in test_roots test_filter_template; do
    V37_SCANNED=$((V37_SCANNED + 1))
    if [ ! -r "$surface" ]; then
      V37_UNREADABLE=$((V37_UNREADABLE + 1))
      continue
    fi
    ...
    if [ "$actual" = "$expected" ]; then
      ...
      V37_FINDINGS=$((V37_FINDINGS + 1))
      _fail "v37 surface scan: $(basename "$(dirname "$surface")")/$(basename "$surface") project.$key is the documented default" "expected: $expected" "  actual: $actual"
    fi
  done
done
V37_SKIPPED=$((V37_SCANNED - V37_CHECKED))
echo "  v37-surface-scan: ${V37_SCANNED} scanned, ${V37_SKIPPED} skipped (unreadable=${V37_UNREADABLE}), ${V37_CHECKED} checked, ${V37_FINDINGS} findings"
assert_eq "v37 surface scan: scanned == skipped + checked" "$V37_SCANNED" "$((V37_SKIPPED + V37_CHECKED))"
assert_eq "v37 surface scan: every surface/key pair was checked" "$V37_CHECKED" "4"
assert_eq "v37 surface scan: migrated config and template agree on the whole project key set" \
  "$(jq -S -c '.project | {test_roots, test_filter_template}' "$NAZGUL_DIR/config.json")" \
  "$(jq -S -c '.project | {test_roots, test_filter_template}' "$REPO_ROOT/templates/config.json")"
assert_eq "v37 surface scan: migration target equals the template's schema_version" \
  "$(jq -r '.schema_version' "$NAZGUL_DIR/config.json")" \
  "$(jq -r '.schema_version' "$REPO_ROOT/templates/config.json")"

# --- Plan D-7, proven rather than assumed: this repo's OWN live config (v34 at
# the time of writing) walks the incremental chain to the terminal version with
# BOTH the v35 stacking keys and the v36 guard key present. The fixture is the
...
if [ -f "$LIVE_CONFIG" ]; then
  ...
  OUTPUT=$(CLAUDE_PLUGIN_ROOT="$REPO_ROOT" "$MIGRATE" "$NAZGUL_DIR" 2>/dev/null) || true
  CFG="$NAZGUL_DIR/config.json"
  assert_json_field "live config copy: reaches terminal $EXPECTED_TERMINAL from v$LIVE_START_VERSION" "$CFG" ".schema_version" "$EXPECTED_TERMINAL"
  assert_eq "live config copy: v36 guards.red_run_evidence present" \
    "$(jq -r '.guards | has("red_run_evidence")' "$CFG")" "true"
  assert_eq "live config copy: v37 project.test_roots present" \
    "$(jq -r '.project | has("test_roots")' "$CFG")" "true"
  assert_eq "live config copy: v37 project.test_filter_template present" \
    "$(jq -r '.project | has("test_filter_template")' "$CFG")" "true"
  assert_eq "live config copy: this repo's own project.test_command survives the walk unfiltered" \
    "$(jq -r '.project.test_command' "$CFG")" "$(jq -r '.project.test_command' "$LIVE_CONFIG")"
  assert_eq "live config copy: v35 execution.stacking present alongside it" \
    "$(jq -r '.execution.stacking | has("enabled")' "$CFG")" "true"
  assert_eq "live config copy: v35 stack.layers present alongside it" \
...
assert_contains "live config walk: disposition recorded as '$LIVE_DISPOSITION' (never a silent skip)" \
  "migrated-a-copy absent" "$LIVE_DISPOSITION"

# --- template-shape assertion: templates/config.json reflects the current schema ---
TEMPLATE_FILE="$REPO_ROOT/templates/config.json"
assert_json_field "template: schema_version is $EXPECTED_TERMINAL" "$TEMPLATE_FILE" ".schema_version" "$EXPECTED_TERMINAL"
assert_json_field "template: guards.subagent_resume is true" "$TEMPLATE_FILE" ".guards.subagent_resume" "true"
assert_json_field "template: guards.in_flight_hold is true" "$TEMPLATE_FILE" ".guards.in_flight_hold" "true"
assert_json_field "template: guards.in_flight_stale_minutes is 30" "$TEMPLATE_FILE" ".guards.in_flight_stale_minutes" "30"
assert_json_field "template: guards.red_run_evidence is true (default-on enforcement gate)" \
...
assert_json_field "template: execution.stacking.max_unmerged is 3" \
  "$TEMPLATE_FILE" ".execution.stacking.max_unmerged" "3"
assert_json_field "template: execution.stacking.rework_priority is 1" \
  "$TEMPLATE_FILE" ".execution.stacking.rework_priority" "1"
assert_eq "template: stack.layers is an empty array" \
  "$(jq -r '.stack.layers | length' "$TEMPLATE_FILE")" "0"
assert_eq "template: guards.team_teardown is absent (dead consumer removed at v33)" \
  "$(jq -r '.guards | has("team_teardown")' "$TEMPLATE_FILE")" "false"
...
assert_eq "template: dead key log_dir is absent" "$(jq -r 'has("log_dir")' "$TEMPLATE_FILE")" "false"
assert_eq "template: dead key review_dir is absent" "$(jq -r 'has("review_dir")' "$TEMPLATE_FILE")" "false"
assert_eq "template: dead key safety.block_destructive_commands is absent" \
  "$(jq -r '.safety | has("block_destructive_commands")' "$TEMPLATE_FILE")" "false"
assert_eq "template: dead key safety.require_tests_pass_before_review is absent" \
  "$(jq -r '.safety | has("require_tests_pass_before_review")' "$TEMPLATE_FILE")" "false"
assert_eq "template: live safety.max_consecutive_failures survives dead-key cleanup" \
  "$(jq -r '.safety.max_consecutive_failures' "$TEMPLATE_FILE")" "5"
assert_eq "template: parallelism.* cluster is retained, not removed" \
  "$(jq -r 'has("parallelism")' "$TEMPLATE_FILE")" "true"
assert_eq "template: context.* cluster is retained, not removed" \
  "$(jq -r 'has("context")' "$TEMPLATE_FILE")" "true"
assert_eq "template: parallelism cluster is marked deprecated" \
  "$(jq -r '._deprecated | has("parallelism")' "$TEMPLATE_FILE")" "true"
assert_eq "template: context cluster is marked deprecated" \
  "$(jq -r '._deprecated | has("context")' "$TEMPLATE_FILE")" "true"

# --- MF-048: migrate-config.sh prunes .bak files to the configured keep-N on the next migration ---
NAZGUL_DIR=$(setup_nazgul_dir "bak-pruning-seeded-12")
cat > "$NAZGUL_DIR/config.json" << 'EOF'
{"schema_version": 28}
EOF
...
BAK_COUNT_BEFORE=$(find "$NAZGUL_DIR" -maxdepth 1 -name '*.bak' | wc -l | tr -d ' ')
...
assert_file_not_exists "8th-oldest pruned .bak file (v8) no longer exists" "$NAZGUL_DIR/config.json.v8.bak"
assert_file_exists "most-recent-among-seeded .bak file (v12) survives pruning" "$NAZGUL_DIR/config.json.v12.bak"
...
assert_json_field "no-positional-arg: fallback migrated the FIXTURE root's config, not cwd's" "$FALLBACK_ROOT/nazgul/config.json" ".schema_version" "$EXPECTED_TERMINAL"
