...
scenario_loop() {
  ...
  # The stop_gate reasons are a closed set enumerated in docs/CONFIGURATION.md -> Telemetry Bus ->
  # Event Types; no live session hits one on demand, so the real hook is driven against its own state.
