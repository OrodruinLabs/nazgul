...
- Notifications: !`tail -20 hydra/notifications.jsonl 2>/dev/null || echo "No notifications"`
...
3. **Notifications** (`hydra/notifications.jsonl`): Each line is a JSON object with `timestamp`, `type`, `message`. Types include: `task_complete`, `review_verdict`, `blocker`, `error`, `milestone`.

4. **Checkpoints** (`hydra/checkpoints/iteration-*.json`): Each file captures a full snapshot at an iteration boundary. Read the latest 5 for context recovery info.
...
- If no iteration logs exist: check if there are at least git commits or notifications. Show whatever is available.
