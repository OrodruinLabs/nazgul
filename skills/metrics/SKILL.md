...
## Examples
- `/hydra:metrics` — View full metrics dashboard
- `/hydra:metrics reviews` — Focus on reviewer stats

## Current State
- Config: !`cat hydra/config.json 2>/dev/null | head -3 || echo "NOT_INITIALIZED"`
- Tasks dir: !`ls hydra/tasks/TASK-*.md 2>/dev/null | wc -l | tr -d ' '`
- Checkpoints dir: !`ls hydra/checkpoints/iteration-*.json 2>/dev/null | wc -l | tr -d ' '`
- Reviews dir: !`ls -d hydra/reviews/TASK-*/ 2>/dev/null | wc -l | tr -d ' '`
...
If Hydra is not initialized, say so and stop.

### Collect Data

Read these sources to compute metrics:

1. **Task manifests** (`hydra/tasks/TASK-*.md`):
...
2. **Checkpoints** (`hydra/checkpoints/iteration-*.json`):
   - Total iterations run
   - First and last iteration timestamps (for time span)
   - Compaction count

3. **Review files** (`hydra/reviews/TASK-*/`):
