...
**Region** = the start-sentinel line through the end-sentinel line, **inclusive**, regardless of blank lines or comments between them. **Legacy fallback**, for a block installed before the end sentinels shipped: the start sentinel through the last consecutive line that is either a `#` comment or a non-blank pattern line, terminating at the first BLANK line or EOF — never at the first comment, which would orphan the block's justification comments. Match both sentinels and every line between them with leading whitespace allowed: blocks written before that version were appended indented.

1. Read `.gitignore`
2. Remove the **local-mode** block if present: the region from `# Nazgul Framework (local mode)` to `# Nazgul Framework — end local mode`. Its entries, for reference: `nazgul/`, `.claude/agents/generated/`, `.mcp.json`.
