...
- **The two hard stops are unconditional — never gated, never yolo-bypassable.** `[enforced]` Any `BLOCKED` task or any non-`APPROVE` `security-reviewer.md` verdict halts the Conductor for a human. `conductor_should_halt` (`scripts/lib/conductor-gates.sh`) fails CLOSED on ambiguity (`BLOCKED_TASKS_AMBIGUOUS`, `SECURITY_REJECTION_AMBIGUOUS`, `*_UNREADABLE`) and ignores every `conductor.gates` value and mode, including `yolo` — this extends §3.5's AFK security-rejection stop and §5's hard-block list into the Conductor engine. `agents/conductor.md` calls it unconditionally at the top of every wave (Step 3.1) and again at every batch boundary within a wave (Step 5.3), so a BLOCKED or security-rejected unit can never let more work start. Same honest limit as the rest of this section: the check runs inside the Conductor's own protocol, not behind a PreToolUse hook — it holds whenever a Conductor session is driving the build.
- **Wave parallelism: Planner-marked and zero-overlap only, capped at `conductor.max_parallel`.** `[hook-driven only]` A wave runs parallel only when `nazgul/plan.md`'s `## Wave Groups` section marks it explicitly AND `route_wave` (`scripts/lib/conductor-router.sh`) finds zero file-scope overlap across the wave's units; any overlap, or an unmarked wave, aborts the whole wave to sequential — the identical rule §8 already gives Team Orchestrator ("Parallel tasks... Zero file overlap... bypassable by manual task dispatch"), not reimplemented here. Batches never exceed `conductor.max_parallel` (default `3`, read via `conductor_max_parallel`). Enforced when `agents/conductor.md` calls `route_wave`/`route_unit` per its Step 4 protocol; bypassable by an orchestrator that dispatches units directly without going through the router — same caveat as §8's Team Orchestrator entry.
