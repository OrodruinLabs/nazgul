...
- **A gate-triggered stop announces itself.** `[hook-driven only]` When the AFK timeout gate ends a run, `scripts/stop-hook.sh` emits a `stop_gate` event to `nazgul/logs/events.jsonl` — fields `reason` (`"afk_timeout"`), `computed` (elapsed minutes), and `limit` (the configured timeout) — before exiting, so a run stopped by a safety gate is distinguishable in telemetry from a run that simply ended (FEAT-023/TASK-001, ADR-014: a mechanism that fails must not look like a mechanism that had nothing to do). FEAT-026/TASK-008 extended the same `stop_gate` event to the in-flight dispatch hold (`reason: "in_flight_hold"`, naming the units and count an ALLOWED, uncounted stop is waiting on) and to a stale marker declined for that hold (`reason: "in_flight_stale"`, naming the unit and its age) — and, since the 2026-08-16 classification fix, to a quarantined non-background marker, under two DISTINCT reasons that must not be collapsed: `reason: "in_flight_orphan"` when the dispatch class was PROVEN (`background: "false"`, or a named dispatch whose report contract owns the marker) — a fresh marker a synchronous dispatch left behind — and `reason: "in_flight_unverifiable"` when the class was NOT OBSERVABLE at marker-write time, which on a fork-mode host is every marker, since `run_in_background` is omitted from the exposed Agent tool schema there. Both name unit/agent/background and both continue the loop normally, but they do DIFFERENT things to the marker, and that difference is the point: `in_flight_orphan` MOVES it to `nazgul/in-flight/quarantine/` (the class was proven, so it is residue), while `in_flight_unverifiable` LEAVES IT IN PLACE (the class was never observed, the dispatch may still be running, and `mv` is irreversible — destroying it would also foreclose #218's fix, which reconciles these markers against the Stop payload's `background_tasks[]`). Only the first asserts a leak, and only the first quarantines. A telemetry consumer counting `in_flight_orphan` as "leaks detected" must not silently absorb the second (#218) — see Config Upgrades / In-Flight Dispatch Hold in `docs/CONFIGURATION.md`. Other soft-limit stops in the table above still do not emit it, and this rule must not be read as claiming they do. FEAT-033 (#218) adds three more arms whose dispositions again differ, and again that difference is the point. `reason: "in_flight_orphan_candidate"` (with `evidence: "background_tasks_empty"`) is the empty-array arm: the Stop payload WAS observed and reports no subagent of any status for this session — a MEASUREMENT, not a proven leak — so the marker is LEFT IN PLACE, nothing is moved, and the reason is deliberately NOT `in_flight_orphan`, which names a PROVEN class that really was quarantined and which `skills/status/SKILL.md` tells operators exactly that about; the irreversible move stays deferred until ADR-027's Q3 measurement bar. That arm is ATTRIBUTION-GATED, and the gate is a precondition of the bar rather than a refinement of it: `background_tasks[]` is THIS session's registry and in-flight markers carry no session id (#248), so under a shared `nazgul/` an empty registry means "nothing live in MY session" and not "this marker's dispatch is not running". When more than one live session is registered, the arm emits `reason: "in_flight_orphan_unattributable"` (`evidence: "shared_nazgul_dir"`, plus `sessions`) instead, and the Q3 tally counts only `in_flight_orphan_candidate` — a DISTINCT reason and not a flag, because a tally keyed on the reason must not be able to absorb an observation it was never entitled to count, and the probe fails CLOSED (a session count that cannot be read is "cannot attribute"). Both detect-only arms fire at ANY marker age and carry `age` (`fresh`|`stale`): running them only inside the freshness branch excluded aged markers, which is very nearly the definition of the orphan population the bar exists to measure, so the instrument was excluding its own target. `in_flight_stale` gains a third form for the same family of reasons: past the far larger ABANDONED ceiling (`_IN_FLIGHT_MARKER_MAX_MIN`, a script constant) a marker on a LIVE tick carries `held_over_age: "false"` plus `abandoned_after:n` and stops counting as awaited work or keying the hold budget, because nothing will ever clear it — which does NOT reopen #211, since on a live tick the hold rests on the payload's live subagents rather than on that marker and IS still taken; only the claim that this marker is what we are waiting for is withdrawn, and the marker itself is still neither moved nor deleted. `reason: "in_flight_hold_budget_exhausted"` is the Q1 hold-budget valve: a second hold on an UNCHANGED marker set (fingerprinted under `nazgul/logs/.in-flight-holds/`) is refused, the loop falls through to the ordinary iteration increment, and it quarantines nothing and moves nothing — its `ledger` field says whether the budget was genuinely `spent` or the ledger was `unwritable`, so a mechanism that FAILED stays distinguishable from one that had nothing to do. `stop_payload_observed` is a new event TYPE, not a `stop_gate` reason — once per Stop the hook processes — emitted above the `guards.in_flight_hold` kill switch but BELOW the `paused` gate, so a paused loop records nothing and a gap in these events is not by itself evidence about the host, carrying `bg_seen`, `types`/`statuses`, and then two MUTUALLY EXCLUSIVE halves — the `entries`/`subagents`/`live` counts on the `yes` arm ONLY, and `why` on the `unknown` arm ONLY, so field PRESENCE carries the meaning and an `unknown` record publishes no count rather than an initialisation constant a Q3 tally could read as an observation — where `why` is a CLOSED seven-member set naming why an `unknown` observation stayed unknown (`no_stdin`, `read_timeout`, `read_timeout_partial`, `not_json`, `field_absent`, `field_wrong_type`, `no_jq`) — so a consumer keying on `stop_gate` will not see it and must not read its absence there as the observation never having happened. The two counts behind these arms are separate on purpose: a LIVE count (subagent type AND an allowlisted `running`/`pending` status) gates the HOLD, a status-blind SUBAGENT_PRESENT count (type only) gates the CANDIDATE, and a status the filter does not recognise produces neither a hold nor a candidate — "neither" is a claim about the two DISPOSITIONS and never about the counts, since such an entry does still increment the status-blind SUBAGENT_PRESENT count, which is what makes present-but-not-live a third STATE rather than an absence — the same "looked and found none" vs "never actually looked" distinction §15/ADR-009 draws for guards, applied here to the filter itself. That third state now has its own record. `reason: "in_flight_present_not_live"` RECORDS AND DOES NOTHING ELSE — a subagent is present with a status the allowlist does not recognise, so there is no hold, no quarantine, and the marker is left untouched. `reason: "in_flight_hold_unbudgetable"` DECLINES the hold: the Q1 ledger could not be written, so the hold could not be BOUNDED, and an unbounded hold is declined rather than taken — it is deliberately NOT `in_flight_hold_budget_exhausted`, which names a budget that worked exactly as designed, because a mechanism that FAILED must stay distinguishable from one that had nothing to do. `in_flight_stale` is likewise reachable on a live tick, carrying `held_over_age: "true"`: #211 forbids a stale bound from DECLINING a hold, never from reporting the possibly-crashed subagent that produced it, so holding on a marker and reporting it are independent. The PROVEN class splits by strength of proof, and FEAT-033's payload observation reaches only one half. `background: "false"` is a mechanical impossibility — a synchronous dispatch cannot span a Stop — so it quarantines regardless of what `background_tasks[]` reports; session liveness is a fact about the session, not about that marker. A NAMED dispatch's proof is contractual, and a named dispatch can be genuinely background and live, so on a tick reporting `LIVE > 0` it is held and named rather than quarantined. The hold's `units` field therefore never names a `background: "false"` marker.
...
- **A dispatch marker names a prompt by digest, and that field has a closed two-state grammar.** `[enforced]` `scripts/in-flight-marker.sh` writes `prompt_hash` plus `prompt_bytes` and never prompt text (FEAT-034/ADR-028 D4–D5; the removed `prompt_head` applied `cut -c1-200` per LINE, so it kept 200 characters of every line rather than 200 in total). `prompt_hash` is EITHER a real digest matching `^[0-9a-f]{16}$` OR the literal token `unavailable`, and one anchored regex decides which: `unavailable` contains `u`, `n`, `v`, `i` and `l`, none of which is a hexadecimal digit, so no digest can ever equal it and a degradation can never be misread as a computed value. That grammar is now enforced AT THE WRITER and not merely published: from #254 A2 the truncated value is matched against `^[0-9a-f]{16}$` and anything else is recorded as `unavailable`. Three shapes were driven against the pre-change writer and each was written verbatim into the marker — a wrapper printing a deprecation line before the digest, an uppercase-hex build, and a short-digest helper — because the writer's only guard was a non-empty test. The rejection names its cause CLASS on stderr (`length=N`, `non-hex-character`) and never the rejected value, which is prompt-derived and therefore covered by ADR-028 D4 like any other prompt byte. `prompt_bytes` is ALWAYS a JSON number and NEVER `null`: `wc -c` over the same byte stream that is hashed, falling back to `${#PROMPT}` re-counted under `LC_ALL=C` — where it counts bytes rather than the characters a UTF-8 locale would give — whenever the `wc -c | tr` pipeline yields no usable count. That stderr line names the TESTED condition and not a cause, because an absent `wc`, an absent `tr`, a signal, and a `wc` that works perfectly but pads with TABs (which `tr -d ' '` does not strip) ALL reach it (#254 C-j). That fallback is a FOURTH state of the grammar, and it is READABLE IN THE ARTIFACT rather than only on stderr: the count is exact and equal to `wc`'s, so the marker carries a third field, **`prompt_bytes_source`, whose closed value set is `wc` or `shell`**, naming which mechanism took the count. Without it states 1 and 4 were byte-identical in the marker and a four-way classifier could never emit its fourth bucket — ADR-014's exact shape (#254 R1). That field is scoped to the BYTES axis and makes NO claim about the hash axis: the two `prompt_hash: "unavailable"` causes remain distinguishable on stderr alone, as before. The marker is a per-dispatch JSON artifact with no version of its own, so the field cost no `schema_version` bump and no config key. Before #254 A1 that path wrote JSON `null` with NO stderr at all, in the same commit that cites ADR-014 — the hash path twelve lines above announced BOTH of its degradations. `e3b0c44298fc1c14` with `prompt_bytes: 0` is a SUCCESSFUL hash of an empty prompt rather than a failure mode. The marker records the state; stderr names the cause (ADR-014 — a mechanism that degraded must not look like one that had nothing to do). Full record in `docs/ARCHITECTURE.md`.
...
- **The registry of bound entry points lives HERE, not in a per-objective TRD.** `[enforced]` Fifteen entry points are bound
  ...
  rather than inherited from its neighbour). The fifteenth is
