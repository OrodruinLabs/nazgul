...
- **`## Merge Evidence` is the only thing that makes `IMPLEMENTED -> DONE` reachable.** `[enforced]` The edge is in `ttg_valid_transition`'s table, but `ttg_validate_transition` refuses it unless `ttg_verify_merge_evidence` validates four fields under that exact heading — `host`, `pr`, `merged-at`, `merge-commit` — each shape-checked, none of them recorded anywhere else in the manifest counting for anything. The heading IS the enforcement boundary, exactly as `## Commits` is. Four closed refusal reasons keep the failures distinguishable and each emits `merge_evidence_missing`: `absent` (no section, or a section with nothing in it), `commented_out` (content present but only inside an HTML comment — a comment is not a record, and this is deliberately not `absent`), `truncated` (a required field is missing), `malformed` (a field is present but fails its shape check). Unlike the red-run gate there is **no kill switch**, and the reason is structural: a switch on the last gate before DONE would be the bypass. The accepting path announces itself too — the route, its fields, and the ancestry outcome go to stderr, so a DONE reached without a review board says so. **What the gate does NOT establish, stated because it is the load-bearing limit:** `ttg_verify_merge_evidence` checks the SHAPE of four fields in a file the implementer can write, and asks no host anything. A hand-typed section with a plausible host, a numeric PR, an ISO timestamp and a hex SHA passes it identically to a real closure. What supplies the fact is the WRITER, not the gate: `scripts/close-objective.sh` is the sole writer of the section, it obtains merge state from the merge-provider seam before writing a byte, and it writes only when the host answered `result: ok` AND reported `merged: true` — an unreachable or unusable host is the named skip `merge-unverifiable`, never a pass, because "could not look" is not "not merged". So the trustworthiness of a `## Merge Evidence` block is the trustworthiness of the path that produced it; the gate's job is to make a MALFORMED or ABSENT one impossible to mistake for a real one, and that is all it does.
...
- **Merge state comes from the host's PR API; git ancestry is corroboration and never a predicate.** `[enforced]` `_ttg_merge_ancestry` records `corroborated`, `squash_signature`, or `unavailable` and every path returns 0 — the outcome reaches the diagnostic and never the verdict. This is not caution, it is correctness: after a server-side squash NO SHA recorded under `## Commits` is an ancestor of the merge commit, so on a squash host a failing ancestry check is the EXPECTED reading and blocking on it would report "not shipped" for work that demonstrably shipped. `scripts/close-objective.sh` and `scripts/lib/merge-provider.sh` are both forbidden from consulting ancestry at all (§16), and a future "simplification" of either into `git merge-base --is-ancestor` is a regression, not a cleanup.
